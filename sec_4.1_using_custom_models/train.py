...
if not params.dummy_run and params.wandb:
    if "gpt" in str(params.pos_path):
        run_name = 'gpt'
    elif "token" in str(params.pos_path):
        run_name = 'bert_case_token'
    else:
        run_name = 'bert_case_sent'
    run_name += "." + str(params.lr) + "." + str(params.batch_size) + "." + str(params.n_epochs)+ "." + str(params.seed)
    wandb.init(project="subword_pos_ner", name=run_name)
...
class SpellingModel(nn.Module):
    def __init__(self):
        ...
        self.n_dims = 100
        self.pos_embeddings = nn.Embedding(45, self.n_dims)
        self.ner_embeddings = nn.Embedding(5, self.n_dims)

        self.num_feats = int(params.ner) + int(params.pos)
    ...
    def forward(self, all_logits):
        pos_logits, ner_logits = all_logits
        pos_ip = torch.matmul(torch.nn.functional.softmax(pos_logits, 1),
                            self.pos_embeddings.weight)
        ner_ip = torch.matmul(torch.nn.functional.softmax(ner_logits, 1),
                            self.ner_embeddings.weight)

        mlp_ip = torch.cat([pos_ip, ner_ip], dim=-1)
        ...
        if not params.dummy_run and params.wandb:
            ...
            for labl in ["Pos"]:
                for metric, val in classify_report[labl].items():
                    if 'f1' in metric.lower() or 'f-1' in metric.lower():
...
if not params.dummy_run and params.wandb:
    ...
    for labl in ["Pos"]:
        for metric, val in classify_report[labl].items():
            if 'f1' in metric:
                wandb_dict["Aggr_Test" + labl + "_" + metric] = val
